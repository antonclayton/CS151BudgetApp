...
public class TransactionSearchPageController {
	...
	private void displaySearchResultT(List<Transaction> filteredTList) {
		...
		if (transactions.size() > 0) {
			for (Transaction transaction : transactions) {
				...
				Label tDescLabel = new Label(transaction.getDescription());
	...
	private void displaySearchResultST(List<ScheduledTransaction> filteredSTList) {
		...
		if (scheduledTransactions.size() > 0) {
			for (ScheduledTransaction scheduledTransaction : scheduledTransactions) {
				...
				Label stNameLabel = new Label(scheduledTransaction.getScheduledTransactionName());
	...
	public void searchTransactions() {
		...
		List<Transaction> filteredTransactions = transactions.stream()
				.filter(transaction -> transaction.getDescription().toLowerCase().contains(searchText.toLowerCase()))
				.toList(); // immutable
	...
	public void searchScheduledTransactions() {
		...
		List<ScheduledTransaction> filteredScheduledTransactions = scheduledTransactions.stream()
				.filter(scheduledTransaction -> scheduledTransaction.getScheduledTransactionName().toLowerCase()
	...
	private void deleteTransaction(Transaction transaction) {
		TransactionsDBHelper.removeTransaction(transaction);
		secondCall = true;
		searchTransactions();
	}

	private void deleteScheduledTransaction(ScheduledTransaction scheduledTransaction) {
		ScheduledTransactionsDBHelper.removeScheduledTransaction(scheduledTransaction);
		secondCall = true;
