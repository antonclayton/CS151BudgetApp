...
public class ScheduledTransactionsDBHelper {
	...
	public static void removeScheduledTransaction(ScheduledTransaction scheduledTransaction) {
		...
		try (PreparedStatement pstmt = DatabaseConnectionManager.getConnection(URL).prepareStatement(sql)) {
			// Set the parameters
			pstmt.setString(1, scheduledTransaction.getScheduledTransactionName());

			// execute update to remove type
			int rowsAffected = pstmt.executeUpdate();

			if (rowsAffected > 0) {
				System.out.println("Type deleted from database: " + scheduledTransaction.getScheduledTransactionName());
			} else {
